...
const changelog = [
  {
    ...
    entries: [
      { type: 'feature', title: 'Employee Profiles', description: 'Full feature set now available to all users.' },
  ...
  },
  {
    ...
    entries: [
      { type: 'launch', title: 'ZiggyHR Launched', description: 'We built ZiggyHR for our own business first. Now available for yours.' },
...
const typeColors: Record<string, string> = { feature: '#22c55e', improvement: '#f59e0b', fix: '#ef4444', launch: '#8b5cf6' }
    ...
    <div className="bg-[#0a0a0a] min-h-screen" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>
      ...
      <section className="pt-20 pb-16 px-4 text-center">
        <div className="max-w-3xl mx-auto">
          <p className="text-xs font-semibold uppercase tracking-widest text-[#8b5cf6] mb-4">Changelog</p>
          <h1 className="text-5xl md:text-6xl font-bold text-white mb-4">What&apos;s new in ZiggyHR</h1>
      ...
      <section className="py-12 px-4">
        <div className="max-w-3xl mx-auto space-y-12">
          {changelog.map((r) => (
            <div key={r.version} className={`bg-[#111111] border rounded-2xl overflow-hidden ${r.highlight ? 'border-[#8b5cf6]/30' : 'border-[#1f1f1f]'}`}>
              <div className={`px-8 py-5 flex items-center justify-between ${r.highlight ? 'bg-[#8b5cf6]/5' : 'bg-[#0d0d0d]'} border-b border-[#1f1f1f]`}>
                <div><p className="text-white font-bold text-lg">{r.version}</p><p className="text-[#b3b3b3] text-sm">{r.date}</p></div>
                {r.highlight && <span className="px-3 py-1 bg-[#8b5cf6] text-white text-xs font-bold rounded-full">Latest</span>}
              </div>
